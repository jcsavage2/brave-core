...
IN_PROC_BROWSER_TEST_F(IpnsManagerBrowserTest, KeysSynchronization) {
  ...
  ipns_manager->SynchronizeKeys(
    base::BindOnce([](base::OnceCallback<void(void)> launch_callback,
         const bool success) {
           ASSERT_TRUE(success);
           if (launch_callback)
...
IN_PROC_BROWSER_TEST_F(IpnsManagerBrowserTest, GenerateKey) {
  ...
  ipns_manager->GenerateNewKey("MyNewKey",
    base::BindOnce([](base::OnceCallback<void(void)> launch_callback,
...
IN_PROC_BROWSER_TEST_F(IpnsManagerBrowserTest, RemoveKey) {
  ...
  ipns_manager->GenerateNewKey("MyNewKey",
    base::BindOnce([](base::OnceCallback<void(void)> launch_callback,
                      bool success, const std::string& name,
