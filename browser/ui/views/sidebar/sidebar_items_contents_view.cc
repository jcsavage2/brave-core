...
SidebarItemsContentsView::SidebarItemsContentsView(
    ...
    SidebarItemController* item_controller)
    : browser_(browser),
...
void SidebarItemsContentsView::AddItemView(const sidebar::SidebarItem& item,
  ...
  auto* item_view = AddChildViewAt(
      std::make_unique<SidebarItemView>(this, item_controller_), index);
...
int SidebarItemsContentsView::DrawDragIndicator(views::View* source,
  ...
  views::View::ConvertPointToScreen(source, &screen_position);
  int target_index = CalculateTargetDragIndicatorIndex(screen_position);
  if (source_view_index == target_index ||
      source_view_index + 1 == target_index) {
    ClearDragIndicator();
  } else {
    DrawDragIndicator(target_index);
  }

  return target_index;
}

void SidebarItemsContentsView::DrawDragIndicator(int index) {
